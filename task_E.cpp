...
void dfs(const Graph &g, std::vector<bool> &used, Graph::Vertex v) {
	used[v] = true;
	for (Graph::Vertex i: g.get_neighbors(v)) {
		if (!used[i]) {
			dfs(g, used, i);
		}
	}
...
int main() {
	...
	std::vector<Graph::Vertex> ts_list = GraphProcessing::top_sort(g);
	std::vector<bool> used(n, false);
	int counter = 0;
	for (auto i = ts_list.rbegin(); i != ts_list.rend(); ++i) {
		if (!used[*i]) {
			++counter;
			dfs(g, used, *i);
		}
