...
namespace GraphProcessing {
	namespace{
		...
		const long int BIG_INF = 2009000999;
	...
	void floyd(int** graph, const int &n) {
		for (int k = 0; k < n; ++k) {
			for (int i = 0; i < n; ++i) {
				for (int j = 0; j < n; ++j) {
	...
	std::vector<long int> Dijsktra(const WeightedGraph &g, const Vertex &start) {	
		std::priority_queue<std::pair<Vertex, long int>, std::vector<std::pair<Vertex, long int>>, cmp_for_prim> q;
		std::vector<long int> d(g.get_vertex_count(), BIG_INF);
		std::vector<bool> used(g.get_vertex_count(), false);
		d[start] = 0;
		q.push({start, 0});
		while (!q.empty()) {
			...
			used[v] = true;
			for (std::pair<Vertex, int> u: g.get_neighbors(v)) {
				if (d[u.first] > d[v] + u.second) {
					d[u.first] = d[v] + u.second;
					q.push({u.first, d[u.first]});
				}
			}
		}
		return d;
	...
	bool is_bipartite(const Graph &g) {
		...
		std::vector<bool> used(g.get_vertex_count(), false);
		std::queue<Vertex> q;
		std::vector<int> part(g.get_vertex_count(), NO_PART);
		for (Vertex v = 0; v < g.get_vertex_count(); ++v) {
			if (!used[v]) {
				q.push(v);
				used[v] = true;
...
void bfs(const Graph &g, int start, std::vector<size_t> &dist) {
	std::vector<bool> used(g.get_vertex_count(), false);
//	std::vector<size_t> dist(n * m, GraphProcessing::INF);
	...
	while (!q.empty()) {
		int u = q.front();
		q.pop();
		for (Graph::Vertex v: g.get_neighbors(u)) {
			if (!used[v]) {
				used[v] = true;
				q.push(v);
				if (dist[u] + 1 < dist[v]) { 
...
int main() {
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < m; ++j) {
			if (j - 1 >= 0) {
				g.add_edge(GetNum(i, j, m) - 1, GetNum(i, j - 1, m) - 1);
			}
			if (j + 1 < m) {
				g.add_edge(GetNum(i, j, m) - 1, GetNum(i, j + 1, m) - 1);
			}
			if (i + 1 < n) {
				g.add_edge(GetNum(i, j, m) - 1, GetNum(i + 1, j, m) - 1);
			}
			if (i - 1 >= 0) {
				g.add_edge(GetNum(i, j, m) - 1, GetNum(i - 1, j, m) - 1);
			}
