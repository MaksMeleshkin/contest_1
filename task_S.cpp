...
namespace GraphProcessing {
	...
	std::vector<int> Ford_Bellman(std::vector<Edge> &edges, const size_t &vertex_count, const Vertex &v) {
		const int INF = 30000;
...
int main() {
	...
	std::vector<int> fb_list = GraphProcessing::Ford_Bellman(edges, n, 0);
	for (int i: fb_list) {
		std::cout << i << " ";
	}
}
